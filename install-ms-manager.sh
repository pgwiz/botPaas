...
MS_MENU_REGISTRY=(
  ...
  "plugin-setup.sh|Plugin setup (add/update menus)"
...
if [ -f "$MENUS_DIR/ref.sh" ] && ! grep -q "plugin-setup.sh" "$MENUS_DIR/ref.sh"; then
    awk 'BEGIN{added=0} /^[[:space:]]*\)[[:space:]]*$/ && !added {print "  \"plugin-setup.sh|Plugin setup (add/update menus)\""; added=1} {print}' "$MENUS_DIR/ref.sh" > "$MENUS_DIR/ref.sh.tmp" && mv "$MENUS_DIR/ref.sh.tmp" "$MENUS_DIR/ref.sh"
    chmod 644 "$MENUS_DIR/ref.sh"
fi
